...
function progressPercent(status: TaskStatusResponse | null): number {
  if (!status) return 0
  ...
  const stepProgress = ((index + 1) / PIPELINE_STEPS.length) * 85
    ...
    <section className="panel" id="pipeline-section">
      <div className="mb-5 flex items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-parchment-200/70">
            Real-time progress from <span className="font-mono text-parchment-400">GET /status</span>.
          </p>
        </div>
        <span
      ...
      {!taskId ? (
        <div className="rounded-xl border border-dashed border-archive-700 bg-archive-950/40 px-6 py-12 text-center">
          <p className="text-sm text-archive-600">Start generation to watch the pipeline.</p>
          <p className="mt-2 text-xs text-archive-700">LLM → LSB encode → archive persist</p>
        </div>
      ) : (
        <div className="space-y-6">
          ...
          {status?.status === 'completed' && status.result ? (
            <div className="animate-fade-in-up space-y-4 rounded-xl border border-emerald-500/30 bg-emerald-500/5 p-4">
              <p className="text-sm font-semibold text-emerald-200">Artifact sealed successfully</p>
              <div className="grid gap-4 lg:grid-cols-[220px_1fr]">
                ...
                <div className="rounded-xl border border-archive-700 bg-archive-950/60 p-4">
                  ...
                  <pre className="max-h-48 overflow-auto whitespace-pre-wrap font-mono text-sm leading-relaxed text-parchment-100">
                    {status.result.riddle ?? status.result.embedded_text ?? 'Riddle embedded.'}
                  </pre>
